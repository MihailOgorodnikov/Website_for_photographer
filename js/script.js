...
const meniToggler = document.querySelector('#menu-toggler');
const navbarMenu = document.querySelector('.navbar__menu');
const toggleMenu = () => {
    navbarMenu.classList.toggle('active');
}
meniToggler.addEventListener('click', toggleMenu);
// --- CLOSE MENU WHEN NAV-LINKS ARE CLICKED ---
const linksToggleMenu = (e) => {
    if(e.target.classList.contains('navbar__list-link'))navbarMenu.classList.remove('active');
    if(e.target.classList.contains('navbar__menu'))navbarMenu.classList.remove('active');
}
...
sr.reveal('.services__images-wrapper, .about__information, .post--left',{
    origin: 'left'
});
